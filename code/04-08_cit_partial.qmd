...
cit_spec <- 
  ...
  decision_tree(tree_depth = tune()) %>%
    # Specify the engine (which package is used for the implementation of cit)
  set_engine("partykit",
             conditional_min_criterion = tune()) %>%
    # Specifying mode - as it can run both cat and num response variable, we need to specify  
  set_mode("regression")
cit_spec
```
...
final_fit %>%
  collect_predictions() # applies to the training set
```
...
final_spec %>%
  ...
  .$fit %>%
  plot()
  ## the model is very sensitive to the data you use to predict. it is up to discussion whether you want to use the whole set or just the training. It is best practice to use the training set
```
